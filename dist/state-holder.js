...
class StateHolder {
    ...
    clear(state, ...keys) {
        ...
        return Object.assign(Object.assign({}, state), keysCleared);
    }
    /**
    ...
    select$(selector, args) {
        ...
        if (args) {
            const newObs = this._stateHolder$.pipe(operators_1.map(state => selector(state, args)), this.processPipe());
            this._selectorsMap[selector.name] = newObs;
            return newObs;
        }
        const selectorWithoutArgs = selector;
        const newObs = this._stateHolder$.pipe(operators_1.map(state => selectorWithoutArgs(state)), this.processPipe());
        this._selectorsMap[selector.name] = newObs;
        return newObs;
    }
    devMode() {
        this._stateHolder$
            ...
            .subscribe({ next: m => { var _a; return console.log({ action: (_a = this._lastActionName) !== null && _a !== void 0 ? _a : 'initial', state: m }); } });
    }
    processPipe() {
        return rxjs_1.pipe(operators_1.filter(d => d !== null && d !== undefined), operators_1.distinctUntilChanged());
    }
}
